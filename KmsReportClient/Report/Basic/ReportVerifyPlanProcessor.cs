...
{
    class ReportVerifyPlanProcessor : AbstractReportProcessor<ReportViolations>
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();

        Dictionary<string, DataGridViewRow> _rows;
        private readonly string[] _forms1 = { "Планы проверок" };

        private readonly string[][] _headers = {
            new[]
            { "Количество за 2024 год" }, //
        };

        private readonly Dictionary<string, string> _headersMap = new Dictionary<string, string>
        {
            { "Планы проверок", "Наименование"},
        };

        public ReportVerifyPlanProcessor(EndpointSoap inClient, List<KmsReportDictionary> reportsDictionary, DataGridView dgv, ComboBox cmb, TextBox txtb, TabPage page) :
            base(inClient, dgv, cmb, txtb, page,
                XmlFormTemplate.VerifyPlan.GetDescription(),
                Log,
        ...
        public override AbstractReport CollectReportFromWs(string yymm)
        {
            var request = new GetReportRequest
            {
                Body = new GetReportRequestBody
                {
                    ...
                    yymm = yymm,
                    reportType = ReportType.VerifyPlan
                }
            };
        ...
        {
            var reportVerifyPlanDto = Report.ReportDataList.SingleOrDefault(x => x.Theme == form);
            if (reportVerifyPlanDto == null)
            {
                return;
            }

            reportVerifyPlanDto.Data = (from DataGridViewRow row in dgvReport.Rows
        ...
        {
            var reportVerifyPlanDto = Report.ReportDataList?.Single(x => x.Theme == form);
            if (reportVerifyPlanDto?.Data == null || reportVerifyPlanDto.Data.Length == 0)
            {
                return;
            }
            ...
            foreach (DataGridViewRow row in dgvReport.Rows)
            {
                ...
                var data = reportVerifyPlanDto.Data.SingleOrDefault(x => x.Code == rowNum);
                if (data != null)
                {
