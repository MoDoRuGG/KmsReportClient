...
{
    public class ReportPgQProcessor : AbstractReportProcessor<ReportPg>
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();

        private readonly string[] _forms1 = { "Таблица 1", "Таблица 11", "Таблица 12" };
        private readonly string[] _forms2 = { "Таблица 2", "Таблица 3" };
        private readonly string[] _forms3 = { "Таблица 4", "Таблица 10", "Таблица 13" };
        private readonly string[] _forms4 = { "Таблица 6", "Таблица 8" };
        ...
        private readonly string[][] _headers = {
            ...
            { "Всего" }, //10
            new[] 
            { "Штатные работники", "Привлекаемые по договору" }, //11
            new[] 
            { "численность опрошенных застрахованных лиц", "из них удовлетворены" }, //12
            new[] 
            { "Всего" }, //13
            new[]
            {
        ...
        private readonly Dictionary<string, string> _headersMap = new Dictionary<string, string>
        {
            ...
            { "Таблица 11", "Специалисты, участвующие в защите прав застрахованных лиц" },
        ...
        public override string ValidReport()
        {
            ...
            string message = "";
            string[] validForms = { "Таблица 2", "Таблица 3", "Таблица 4", "Таблица 6", "Таблица 8" };
            ...
            foreach (var data in Report.ReportDataList.Where(x => validForms.Contains(x.Theme)))
            {
                ...
                else if (data.Theme == "Таблица 6" || data.Theme == "Таблица 8")
                {
        ...
        public override void ToExcel(string filename, string filialName)
        {
            ...
            var excel = new ExcelPgQCreator(filename, ExcelForm.PgQ, mm, filialName);
        ...
        private void FillDgwForms6(DataGridView dgvReport, string form)
        {
            ...
            foreach (DataGridViewRow row in dgvReport.Rows)
            {
                ...
                row.Cells[2].Value = PgDgvUtils.GetRowText(isExclusionsRow, exclusionsCells, 2, data.CountAmbulatory);
                row.Cells[3].Value = PgDgvUtils.GetRowText(isExclusionsRow, exclusionsCells, 3, data.CountStac);
                row.Cells[4].Value = PgDgvUtils.GetRowText(isExclusionsRow, exclusionsCells, 4, data.CountDs);
                row.Cells[5].Value = PgDgvUtils.GetRowText(isExclusionsRow, exclusionsCells, 5, data.CountOutOfSmoAnother);
                row.Cells[6].Value = PgDgvUtils.GetRowText(isExclusionsRow, exclusionsCells, 6, data.CountSmo);
