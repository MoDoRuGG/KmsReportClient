...
{
    class ReportViolEKMPProcessor : AbstractReportProcessor<ReportViolations>
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();

        Dictionary<string, DataGridViewRow> _rows;
        private readonly string[] _forms1 = { "Нарушения ЭКМП" };

        private readonly string[][] _headers = {
            new[]
            { "За отчетный период" }, //ЭКМП
        };

        private readonly Dictionary<string, string> _headersMap = new Dictionary<string, string>
        {
            { "Нарушения ЭКМП", "Перечень оснований для отказа в оплате медицинской помощи"},
        };

        public ReportViolEKMPProcessor(EndpointSoap inClient, List<KmsReportDictionary> reportsDictionary, DataGridView dgv, ComboBox cmb, TextBox txtb, TabPage page) :
            base(inClient, dgv, cmb, txtb, page,
                XmlFormTemplate.ViolEKMP.GetDescription(),
                Log,
        ...
        public override AbstractReport CollectReportFromWs(string yymm)
        {
            var request = new GetReportRequest
            {
                Body = new GetReportRequestBody
                {
                    ...
                    yymm = yymm,
                    reportType = ReportType.ViolEKMP
                }
            };
        ...
        public override void ToExcel(string filename, string filialName)
        {
            ...
            var excel = new ExcelViolEKMPCreator(filename, ExcelForm.ViolEKMP, mm, filialName);
        ...
        {
            var reportViolEKMPDto = Report.ReportDataList.SingleOrDefault(x => x.Theme == form);
            if (reportViolEKMPDto == null)
            {
                return;
            }

            reportViolEKMPDto.Data = (from DataGridViewRow row in dgvReport.Rows
        ...
        {
            var reportViolEKMPDto = Report.ReportDataList?.Single(x => x.Theme == form);
            if (reportViolEKMPDto?.Data == null || reportViolEKMPDto.Data.Length == 0)
            {
                return;
            }
            ...
            foreach (DataGridViewRow row in dgvReport.Rows)
            {
                ...
                var data = reportViolEKMPDto.Data.SingleOrDefault(x => x.Code == rowNum);
                if (data != null)
                {
